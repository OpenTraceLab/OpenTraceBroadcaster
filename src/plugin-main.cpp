...
static void *measurement_overlay_create(obs_data_t *settings, obs_source_t *source)
{
	...
	context->reader->start(device ? device : "", driver ? driver : "", conn ? conn : "",
...
static void measurement_overlay_video_tick(void *data, float seconds UNUSED)
{
	...
	if (context->reader->get_latest_measurement(value, unit)) {
		...
		if (context->show_units) {
			...
			case 10001:
				unit_str = " A";
				break; // OTC_MQ_CURRENT
			case 10002:
				unit_str = " Ω";
				break; // OTC_MQ_RESISTANCE
			case 10003:
				unit_str = " F";
				break; // OTC_MQ_CAPACITANCE
			case 10004:
				unit_str = " H";
				break; // OTC_MQ_INDUCTANCE
			case 10005:
				unit_str = " Hz";
				break; // OTC_MQ_FREQUENCY
			default:
				unit_str = " V";
				break; // OTC_MQ_VOLTAGE
			}
		}

		snprintf(context->display_text, sizeof(context->display_text), "%.*f%s", context->precision, value,
			 unit_str);
	}
}
...
static obs_properties_t *measurement_overlay_get_properties(void *data)
{
	...
	obs_property_list_add_string(device_list, obs_module_text("MeasurementOverlay.AutoDetect"), "");
	...
	obs_properties_add_text(props, "driver", obs_module_text("MeasurementOverlay.Driver"), OBS_TEXT_DEFAULT);
	obs_properties_add_text(props, "conn", obs_module_text("MeasurementOverlay.Connection"), OBS_TEXT_DEFAULT);
	obs_properties_add_text(props, "serialcomm", obs_module_text("MeasurementOverlay.SerialConfig"),
				OBS_TEXT_DEFAULT);

	// Display settings
	obs_properties_add_int_slider(props, "width", obs_module_text("MeasurementOverlay.Width"), 200, 800, 50);
	obs_properties_add_int_slider(props, "height", obs_module_text("MeasurementOverlay.Height"), 50, 200, 10);
	obs_properties_add_int_slider(props, "font_size", obs_module_text("MeasurementOverlay.FontSize"), 12, 48, 2);
	obs_properties_add_int_slider(props, "precision", obs_module_text("MeasurementOverlay.Precision"), 0, 6, 1);

	obs_properties_add_bool(props, "show_units", obs_module_text("MeasurementOverlay.ShowUnits"));
