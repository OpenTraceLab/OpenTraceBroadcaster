...
std::vector<DeviceInfo> MeasurementReader::scan_devices()
{
	...
	for (GSList *l = all_devices; l; l = l->next) {
		...
		info.display_name =
			std::string(vendor ? vendor : "Unknown") + " " + std::string(model ? model : "Device");
		if (conn) {
...
bool MeasurementReader::start(const std::string &device_id, const std::string &driver, const std::string &conn,
			      const std::string &serialcomm)
{
	if (running)
		return true;
				      ...
				      const struct otc_datafeed_packet *packet, void *cb_data)
{
	...
	if (packet->type == OTC_DF_ANALOG) {
		const struct otc_datafeed_analog *analog = (const struct otc_datafeed_analog *)packet->payload;
